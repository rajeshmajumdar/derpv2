...
public:
  ...
  void pushNotification(const NotificationType &notificationType,
                        const QString &msg) override;

  void switchToModule(const QString &moduleId) override;
  void launchIntent(const QString &moduleId, const QString &method,
                    const QVariantMap &data) override;

  QVariant callService(const QString &moduleId, const QString &method,
                       const QVariantMap &params) override;

  void httpGet(const QString &url, const QVariantMap &data,
               bool isAuthenticated,
               std::function<void(QJsonDocument)> callback) override;
  void httpPost(const QString &url, const QVariantMap &data,
                bool isAuthenticated,
                std::function<void(QJsonDocument)> callback) override;
  void httpPut(const QString &url, const QVariantMap &data,
               bool isAuthenticated,
               std::function<void(QJsonDocument)> callback) override;
  void httpDownload(const QString &url, const QString &destPath,
